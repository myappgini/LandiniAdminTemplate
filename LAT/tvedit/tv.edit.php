...
if(function_exists($bu)) {
    $args=array();
    if(!$bu($data, getMemberInfo(), $args)) { return false; }
}

$o=array('silentErrors' => true);
$update = 'update `'.$tn.'` set `'.$fn[1].'`=' . (($data[$fn[1]] !== '' && $data[$fn[1]] !== NULL) ? "'{$data[$fn[1]]}'" : 'NULL') . '' .  " where `id`='".makeSafe($selected_id)."'";
...
if(function_exists($au)) {
    ...
    $args = array();
    if(!$au($data, getMemberInfo(), $args)) { return; }
}
